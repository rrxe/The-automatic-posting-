...
export function createTelegramOidcUrl(
  telegramId: number
) {
  const clientId =
    requiredEnv(
  ...
  const redirectUri =
    `${webUrl.replace(/\/$/, "")}/auth/telegram/callback`;
...
export async function consumeTelegramOidcCallback(
  code: string,
  state: string
) {
  const entry =
