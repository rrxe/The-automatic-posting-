...
async function sendAdminSettings(
  ctx: BotContext
) {
  const settings =
...
export async function handleAdminExtraCallback(
  ctx: BotContext
): Promise<boolean> {
  if (!ctx.from) {
    return false;
    ...
    > = {
      ...
      set_md: {
        ...
        action:
          "set_message_delay",
        text:
          "🐢 أرسل التأخير بين كل رسالة ووجهة بالدقائق."
      },
      ...
      set_r7: {
        ...
        action:
          "set_referral_7",
        text:
          "🎁 أرسل عدد أيام VIP لمكافأة 7 إحالات."
      },
...
export async function handleAdminExtraText(
  ctx: BotContext,
  text: string
): Promise<boolean> {
  if (!ctx.from) {
  ...
  if (selected) {
    ...
    if (
      ...
      await ctx.reply(
        "❌ يجب أن يكون الرقم صحيحاً بدون فاصلة."
      );
      return true;
    }
    ...
    if (
      ...
      await ctx.reply(
        `❌ القيمة يجب أن تكون بين ${selected.min} و ${selected.max}.`
      );
      return true;
    }
