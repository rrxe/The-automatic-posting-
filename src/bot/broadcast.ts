...
import type { BroadcastContent } from "../services/broadcast.js";
...
function setPending(telegramId: number, content: BroadcastContent) {
  pendingBroadcasts.set(telegramId, { content, createdAt: Date.now() });
}

function getPending(telegramId: number) {
...
export async function startBroadcastCompose(ctx: BotContext) {
  if (!ctx.from) return;
...
async function showPreview(
  ...
  await ctx.reply(
    "👆 هذه معاينة الرسالة تماماً كما راح تصل للمستخدمين.\n\n" +
 ...
 */
export async function handleBroadcastComposeText(
  ctx: BotContext,
  text: string
) {
  if (!ctx.from) return;
  ...
  if (trimmed.length > MAX_TEXT_LENGTH) {
    await ctx.reply(
      `❌ النص أطول من الحد المسموح (${MAX_TEXT_LENGTH} حرف).`
    );
    return;
  }
 ...
 */
export async function handleBroadcastPhoto(
  ctx: BotContext
): Promise<boolean> {
  if (!ctx.from || !ctx.message?.photo?.length) return false;

  const action = await getAdminAction(ctx.from.id);
  if (action !== "broadcast_compose") return false;

  const caption = ctx.message.caption?.trim() || "";

  if (caption.length > MAX_CAPTION_LENGTH) {
    await ctx.reply(
      `❌ التسمية التوضيحية أطول من الحد المسموح (${MAX_CAPTION_LENGTH} حرف).`
    );
    return true;
  }
...
export async function confirmBroadcast(ctx: BotContext) {
  if (!ctx.from) return;
  ...
  await ctx.reply(
    "🚀 بدأ إرسال الرسالة الجماعية بالخلفية.\n\n" +
...
export async function cancelBroadcast(ctx: BotContext) {
  if (!ctx.from) return;
