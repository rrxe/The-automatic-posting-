...
import { updateAppSetting } from "../services/settings.js";
import { isAdmin } from "../services/admin.js";
import { getUserAction, clearUserAction, setUserAction } from "../services/userActions.js";
import {
  beginLogin,
  submitLoginCode,
  submitLoginPassword,
...
export async function handleAdminText(ctx: BotContext) {
  ...
  // ===== إنشاء المنشور =====
  if (await handlePostText(ctx, text)) return;
  ...
  if (adminAction === "vip_grant") {
    ...
    try {
      const { grantVip } = await import("../services/adminManagement.js");
      const expires = await grantVip(telegramId, targetId, days);
  ...
  if (adminAction === "add_mandatory_channel") {
    try {
      ...
      const errors: Record<string, string> = {
        ...
        BOT_NOT_IN_CHANNEL: "❌ البوت غير موجود في القناة.",
  ...
  if (adminAction === "admin_add") {
    ...
    try {
      const { addAdmin } = await import("../services/adminManagement.js");
      await addAdmin(targetTelegramId);
      await clearAdminAction(telegramId);
      await ctx.reply(
  ...
  const settingMap: Record<string, {
    key: "free_group_limit" | "vip_group_limit" | "free_account_limit" | "vip_account_limit" |
          "free_message_limit" | "vip_message_limit" | "free_daily_runs" | "vip_daily_runs" |
          "free_cycle_limit" | "vip_cycle_limit" | "free_cycle_delay_minutes" | "vip_cycle_delay_minutes" |
          "message_delay_minutes" | "referral_7_vip_days" | "referral_20_vip_days" | "vip_price_usdt";
    label: string;
    integer: boolean;
    min: number;
    max: number;
  ...
  if (setting) {
    ...
    try {
      await updateAppSetting(setting.key, value);
      await clearAdminAction(telegramId);
      await ctx.reply(
        "✅ تم التحديث بنجاح.\n\n" +
        `⚙️ ${setting.label}\n` +
      ...
      await ctx.reply("❌ فشل حفظ الإعداد في قاعدة البيانات.");
    }
    return;
  }
}
