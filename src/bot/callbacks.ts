...
import {
  ...
  cancelAllActions
} from "../services/actionManager.js";
...
async function showDeleteChannels(ctx: BotContext) {
  ...
  for (const channel of channels) {
    keyboard
      .text(
        `🗑 ${channel.title || "القناة"}`,
        `admin_delete_channel:${channel.id}`
      )
      .row();
  }
...
export async function handleCallbacks(
  ...
  /*
   * كل Callback خاص بالتنقل:
   * نحذف الشاشة السابقة حتى لا تتراكم
   * عشرات رسائل البوت.
   *
   * رسائل الإدخال لا تمر من هنا، لذلك:
  ...
  if (
    await handleAdminExtraCallback(ctx)
  ) {
    return;
  }
  ...
  if (
    action?.startsWith("pm:")
  ) {
    await chooseSavedMessage(
      ctx,
      action.slice(3)
    );
    return;
  ...
  if (
    ...
    if (
      ...
      await ctx.answerCallbackQuery({
        text:
          `⚠️ وصلت إلى حد التشغيل اليومي ${dailyLimit}.`,
        show_alert:
          true
      });
  ...
  if (
    action?.startsWith("publish_saved:")
  ) {
    const messageId =
      action.slice("publish_saved:".length);
    ...
    if (
      ...
      await ctx.answerCallbackQuery({
        text:
          "❌ اختر مجموعة واحدة على الأقل أولاً.",
        show_alert:
          true
      });
  ...
  if (
    await handleAccountGroupCallback(ctx)
  ) {
    return;
  }
  ...
  if (action?.startsWith("post_edit:")) {
    ...
    await import("../services/userActions.js").then(
      ({ setUserAction }) =>
        setUserAction(ctx.from!.id, "post_content")
    );

    await ctx.reply(
      "✏️ أرسل نص المنشور الجديد."
    );
  ...
  if (action && settingsActions[action]) {
    ...
    const messages: Record<string, string> = {
      setting_free_limit:
        "📣 أرسل الآن حد المجموعات للمستخدم المجاني.\n\nمثال: 5",
      setting_vip_limit:
        "⭐ أرسل الآن حد المجموعات لـ VIP.\n\nمثال: 20",
      setting_ref7:
        "🎁 أرسل عدد أيام VIP التي يحصل عليها المستخدم عند 7 إحالات.",
      setting_ref20:
        "🎁 أرسل عدد أيام VIP التي يحصل عليها المستخدم عند 20 إحالة.",
      setting_price:
        "💰 أرسل سعر VIP الشهري بالـ USDT.\n\nمثال: 5"
    };
  ...
  if (action?.startsWith("ps:")) {
    ...
    const {
      toggleDraftSignature
    } = await import(
      "../services/postDrafts.js"
    );
  ...
  switch (action) {
    case "account_add": {
      ...
      if (
        ...
        await ctx.answerCallbackQuery({
          text:
            "⚠️ وصلت إلى الحد المسموح للحسابات.",
          show_alert: true
        }).catch(() => {});

        await ctx.reply(
          "⚠️ وصلت إلى حد الحسابات في باقتك.\n\n" +
          "Free: حسابان فقط\n" +
          "VIP: حتى 5 حسابات."        );
      ...
      const webBase =
        (
          process.env.WEB_URL ||
          "https://YOUR-MODVC-DOMAIN"
        ).replace(/\/$/, "");

      const loginUrl =
        `${webBase}/telegram-login?token=${encodeURIComponent(webLoginToken)}`;

      await startUserActionExclusive(
        ctx.from.id,
        "telegram_web_login"
      );
