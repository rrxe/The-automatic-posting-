  ...
  if (action?.startsWith("pstart:")) {
    ...
    try {
      const run = await createPublishRun(
        ...
        message.id,
        groups.map((row: any) => row.group_id)
      );
      ...
      void enqueuePublish(() =>
        executePublishCycles(user.id, message.telegram_account_id, run.runId, cycleLimit, delay)
      )
        .then(async (result) => {
          try {
            await ctx.reply(
...
if (action === "admin_admins") {
  ...
  if (action?.startsWith("stop:")) {
     ...
     */
    await ctx.reply(
      stopped ? "⏹ تم طلب إيقاف عملية النشر." : "ℹ️ لا توجد عملية قيد التشغيل.",
      {
        reply_markup: new InlineKeyboard().text("🏠 الرئيسية", "dashboard")
      }
    );
  ...
  switch (action) {
    case "account_add": {
      ...
      if (accounts.length >= 1) {
        ...
        await ctx.reply(
          "⚠️ يُسمح بربط حساب Telegram واحد فقط لكل مستخدم.\n\n" +
