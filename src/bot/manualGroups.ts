...
export async function showManualGroups(
  ctx: BotContext,
  accountId: string
) {
  if (!ctx.from) return;
...
export async function toggleManualGroup(
  ctx: BotContext,
  accountId: string,
  index: number
) {
  ...
  if (!row.is_active) {
    ...
    if (
      ...
      await ctx.answerCallbackQuery({
        text:
          `⚠️ الحد المسموح في باقتك هو ${limit}.`,
        show_alert:
          true
      });
...
export async function detachManualGroup(
  ctx: BotContext,
  accountId: string,
  index: number
) {
