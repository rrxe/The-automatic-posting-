...
interface PendingLogin {
  ...
  stormUserId: string;
  phone: string;
  status: "waiting_code" | "waiting_password" | "completed" | "failed";
  codeResolver: ((code: string) => void) | null;
  passwordResolver: ((password: string) => void) | null;
  waitResolver: ((result: LoginStepResult) => void) | null;
  error: Error | null;
...
export type LoginStepResult =
  | { status: "password" }
  | { status: "completed"; account: unknown }
...
async function saveTelegramAccount(pending: PendingLogin) {
  ...
  if (!session) throw new Error("SESSION_SAVE_FAILED");
  ...
  if (error) {
    if (error.code === "23505") throw new Error("ACCOUNT_ALREADY_LINKED");
    throw new Error(error.message);
  }
  return data;
}

function finishWait(telegramId: number, result: LoginStepResult) {
...
export async function submitLoginCode(telegramId: number, code: string): Promise<LoginStepResult> {
  ...
  if (!resolver) throw new Error("CODE_NOT_REQUESTED");
...
export async function submitLoginPassword(telegramId: number, password: string): Promise<LoginStepResult> {
  ...
  if (!resolver) throw new Error("PASSWORD_NOT_REQUESTED");
...
export function hasPendingLogin(telegramId: number) {
