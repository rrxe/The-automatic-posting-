...
async function deleteWebhookSafely() {
    ...
    try {
        const timeoutPromise = new Promise((resolve) => {
            timer = setTimeout(() => {
                console.warn("WEBHOOK DELETE TIMEOUT — continuing startup.");
                resolve();
            }, 8000);
        });
        const deletePromise = bot.api
            ...
            .catch((error) => {
            console.error("WEBHOOK DELETE FAILED — continuing without crashing:", error instanceof Error
                ? error.message
                : String(error));
        });
        await Promise.race([
...
async function startTelegramBot() {
    ...
    while (true) {
        ...
        try {
            ...
            console.error(`TELEGRAM STARTUP FAILED (attempt #${attempt}) — retrying in 10 seconds:`, error instanceof Error
                ? error.message
                : String(error));
            await sleep(10000);
        }
