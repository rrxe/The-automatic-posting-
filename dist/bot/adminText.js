...
import { getAdminAction, clearAdminAction, addMandatoryChannel } from "../services/adminChannels.js";
...
export async function handleAdminText(ctx) {
    ...
    if (userAction === "telegram_web_login") {
    ...
    if (userAction === "telegram_phone") {
        try {
            ...
            /*
             * لا نمسح الحالة. التسجيل مستمر في الخلفية وينتظر الكود.
             */
    ...
    if (adminAction === "vip_revoke") {
        ...
        try {
            ...
            const message = error instanceof Error ? error.message : "";
            await ctx.reply(message === "USER_NOT_FOUND"
                ? "❌ لم أجد هذا المستخدم."
                : "❌ تعذر سحب VIP.");
        }
        return;
    ...
    if (adminAction === "vip_search" || adminAction === "admin_user_search") {
        ...
        const activeVip = found.plan === "vip" &&
