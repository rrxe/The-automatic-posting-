export async function showScreen(ctx, text, options = {}) {
    /*
     * Callback = صفحة تنقل:
...
export async function editScreen(ctx, text, options = {}) {
    if (!ctx.callbackQuery?.message) {
        return showScreen(ctx, text, options);
    }
    try {
        await ctx.api.editMessageText(ctx.callbackQuery.message.chat.id, ctx.callbackQuery.message.message_id, text, {
            reply_markup: options.reply_markup,
...
export async function deleteCurrentScreen(ctx) {
    if (!ctx.callbackQuery?.message) {
        return;
    }
    try {
