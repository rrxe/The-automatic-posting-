...
import { getUserTelegramAccounts, removeTelegramAccount } from "../telegram/clientManager.js";
import { getAppSettings } from "../services/settings.js";
import { getDailyPublishCount } from "../services/posts.js";
import { showManualGroups, toggleManualGroup } from "./manualGroups.js";
export async function accountPanel(ctx) {
    if (!ctx.from)
        return;
...
export async function accountView(ctx, accountId) {
    if (!ctx.from)
        return;
    const user = await getUserByTelegramId(ctx.from.id);
    if (!user)
    ...
    if (!account) {
        await ctx.reply("❌ الحساب غير موجود.", {
            reply_markup: new InlineKeyboard()
                .text("🏠 الرئيسية", "dashboard")
        });
        return;
    ...
    await ctx.reply("⚙️ حسابي\n\n" +
        `👤 الحساب: ${account.username
            ...
            : account.display_name ||
                "بدون اسم"}\n` +
        `📞 الهاتف: ${account.phone_hint ||
            "مخفي"}\n\n` +
        `📦 الباقة: ${vip
            ? "⭐ VIP"
            : "🆓 مجاني"}\n` +
        `▶️ التشغيلات: ${dailyUsed} / ${dailyLimit}\n` +
        `⏳ المتبقي: ${dailyRemaining}\n` +
        `🔄 حد الدورات: ${cycleLimit}\n` +
        `⏱ الانتظار بين الدورات: ${cycleDelay} دقيقة\n\n` +
        "اختر ما تريد:", {
        reply_markup: new InlineKeyboard()
            .text("▶️ تشغيل النشر", `run:${account.id}`)
            .row()
            .text("📣 مجموعاتي", `ag:${account.id}`)
            .row()
            .text("➕ إضافة مجموعة", `ac:${account.id}`)
            .row()
...
export async function showGroups(ctx, accountId) {
    await showManualGroups(ctx, accountId);
}
export async function syncGroups(ctx, accountId) {
    await ctx.answerCallbackQuery({
        text: "ℹ️ هذه القائمة تعرض المجموعات المضافة يدوياً فقط."
    }).catch(() => { });
    await showManualGroups(ctx, accountId);
}
export async function toggleGroup(ctx, accountId, index) {
    await toggleManualGroup(ctx, accountId, index);
}
export async function removeAccount(ctx, accountId) {
    if (!ctx.from)
        return;
    const user = await getUserByTelegramId(ctx.from.id);
    if (!user)
        return;
    await removeTelegramAccount(accountId, user.id);
    await ctx.answerCallbackQuery({
        text: "✅ تمت إزالة الحساب."
    }).catch(() => { });
