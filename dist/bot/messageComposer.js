...
const SIGNATURE = "نشر تلقائي: @postrush_bot";
function isVip(user) {
 ...
 * ==============================
 */
export async function startMessageComposer(ctx) {
    if (!ctx.from)
        return;
...
export async function chooseMessageAccount(ctx, accountId) {
    if (!ctx.from)
        return;
    const user = await getUserByTelegramId(ctx.from.id);
    if (!user)
...
export async function startNewMessage(ctx, accountId) {
    if (!ctx.from)
        return;
    const user = await getUserByTelegramId(ctx.from.id);
    if (!user)
    ...
    await setUserAction(ctx.from.id, `message_content:${accountId}`);
    const settings = await getAppSettings();
    const limit = isVip(user)
        ? settings.vip_message_limit
        : settings.free_message_limit;
    await ctx.answerCallbackQuery();
    await ctx.reply("✍️ إنشاء منشور جديد\n\n" +
        `📦 الباقة: ${isVip(user)
            ? "⭐ VIP"
            : "🆓 مجاني"}\n` +
        `🔢 الحد: ${limit} حرف\n\n` +
        ...
        reply_markup: new InlineKeyboard()
            .text("↩️ رجوع", `mc:${accountId}`)
            .row()
            .text("🏠 الرئيسية", "dashboard")
    });
}
export async function handleMessageText(ctx, text) {
    if (!ctx.from) {
        return false;
    }
    const { getUserAction } = await import("../services/userActions.js");
    const action = await getUserAction(ctx.from.id);
    if (!action?.startsWith("message_content:")) {
        return false;
    }
    const accountId = action.slice("message_content:".length);
    const user = await getUserByTelegramId(ctx.from.id);
    if (!user) {
        return true;
    }
    ...
    if (!content) {
        await ctx.reply("❌ المنشور لا يمكن أن يكون فارغاً.");
        return true;
    }
    if (content.length >
        limit) {
        await ctx.reply(`❌ المنشور أطول من الحد.\n\n` +
            `🔢 الحد: ${limit}\n` +
    ...
    if (error || !data) {
        ...
        await ctx.reply("❌ تعذر حفظ المنشور.");
        return true;
    }
    await clearUserAction(ctx.from.id);
    await ctx.reply("✅ تم حفظ المنشور بنجاح.\n\n" +
 ...
 * ==============================
 */
export async function startPublishFlow(ctx) {
    if (!ctx.from)
        return;
    const user = await getUserByTelegramId(ctx.from.id);
    if (!user)
...
export async function showSavedMessages(ctx, accountId) {
    if (!ctx.from)
        return;
    const user = await getUserByTelegramId(ctx.from.id);
    if (!user)
    ...
    if (!data?.length) {
        await ctx.reply("📭 لا توجد منشورات محفوظة لهذا الحساب.", {
            reply_markup: new InlineKeyboard()
                .text("✍️ إنشاء منشور جديد", `mnew:${accountId}`)
                .row()
    ...
    data.forEach((message, index) => {
         ...
         */
        keyboard
            .text(`▶️ ${index + 1}. ${preview}`, `pm:${message.id}`)
            .row();
    });
    keyboard
        .text("↩️ رجوع", "run_publish")
        .row()
        .text("🏠 الرئيسية", "dashboard");
    await ctx.reply("▶️ تشغيل النشر\n\n" +
...
export async function chooseSavedMessage(ctx, messageId) {
    if (!ctx.from)
        return;
    const user = await getUserByTelegramId(ctx.from.id);
    if (!user)
    ...
    if (!groups?.length) {
        ...
        await ctx.reply("📣 لا توجد مجموعات مضافة لهذا الحساب.\n\n" +
            "اضغط «➕ إضافة مجموعة» لإضافة وجهة.", {
            reply_markup: new InlineKeyboard()
                .text("➕ إضافة مجموعة", `ac:${message.telegram_account_id}`)
                .row()
                .text("🏠 الرئيسية", "dashboard")
        });
        return;
    }
    const selectedGroups = groups.filter((group) => group.is_active);
    await ctx.answerCallbackQuery();
    await ctx.reply("🚀 مراجعة النشر\n\n" +
        `📝 المنشور:\n${message.content}\n\n` +
        "التوقيع:\n" +
        SIGNATURE +
        "\n\n" +
        ...
        reply_markup: new InlineKeyboard()
            .text("🚀 نشر الآن", `publish_saved:${message.id}`)
            .row()
            .text("📣 مجموعاتي", `ag:${message.telegram_account_id}`)
            .row()
            .text("↩️ المنشورات", `rp:${message.telegram_account_id}`)
            .row()
            .text("🏠 الرئيسية", "dashboard")
    });
}
export async function showMyMessages(ctx, accountId) {
    if (!ctx.from)
        return;
    const user = await getUserByTelegramId(ctx.from.id);
    if (!user)
...
export async function viewMyMessage(ctx, messageId) {
    if (!ctx.from)
        return;
    const user = await getUserByTelegramId(ctx.from.id);
    if (!user)
    ...
    const { data: message, error } = await supabase
        .from("messages")
        .select("id,content,telegram_account_id,status")
        .eq("id", messageId)
        .eq("user_id", user.id)
        .single();
    if (error ||
    ...
    await ctx.answerCallbackQuery();
    await ctx.reply("📝 تفاصيل المنشور\n\n" +
        "────────────\n" +
        message.content +
        "\n────────────\n\n" +
        "التوقيع:\n" +
