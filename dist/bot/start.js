...
function buildMembershipKeyboard(results) {
    const keyboard = new InlineKeyboard();
    for (const item of results) {
        const url = item.channel.invite_url ||
            (item.channel.username
                ? `https://t.me/${item.channel.username.replace(/^@/, "")}`
                : null);
        if (url) {
            keyboard
                .url(`📢 ${item.channel.title || item.channel.username || "القناة"}`, url)
                .row();
        }
    }
...
export async function handleStart(ctx) {
    ...
    const membership = await checkAllRequiredChannels(bot, ctx.from.id);
    if (!membership.complete) {
        const joined = membership.results.filter((item) => item.joined).length;
        await ctx.reply("🔒 الاشتراك مطلوب للمتابعة\n\n" +
            "اشترك في جميع القنوات المطلوبة، ثم اضغط على زر التحقق.\n\n" +
            `✅ تم الاشتراك: ${joined}/${membership.results.length}`, {
            reply_markup: buildMembershipKeyboard(membership.results)
        });
        return;
    }
    const referralCount = await confirmReferral(user);
    if (user.referred_by) {
        await processReferralRewards(user.referred_by, referralCount);
    }
