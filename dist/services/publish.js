...
async function resolveTargetEntity(client, group) {
    if (group.username &&
    ...
    const rawId = String(group.telegram_chat_id);
    if (!rawId) {
        throw new Error("TELEGRAM_TARGET_ID_MISSING");
    }
    try {
...
export async function createPublishRun(userId, accountId, messageId, selectedGroupIds) {
    if (!selectedGroupIds.length) {
        throw new Error("NO_SELECTED_GROUPS");
    }
    ...
    const text = buildPreview(message.content, settings.signature, true);
    return {
        runId: run.id,
...
export async function executePublishCycles(userId, accountId, runId, cycleLimitOverride, cycleDelayOverride) {
    ...
    if (run.status !==
        "pending") {
        throw new Error("RUN_NOT_PENDING");
    }
    const { data: message, error: messageError } = await supabase
        .from("messages")
        .select(`
    ...
    const configuredCycleLimit = vip
        ? settings.vip_cycle_limit
        : settings.free_cycle_limit;
    const configuredCycleDelay = vip
        ? settings.vip_cycle_delay_minutes
        : settings.free_cycle_delay_minutes;
    const cycleLimit = typeof cycleLimitOverride === "number" &&
        cycleLimitOverride > 0
        ? Math.min(Math.floor(cycleLimitOverride), configuredCycleLimit)
        : configuredCycleLimit;
    const cycleDelay = typeof cycleDelayOverride === "number" &&
        cycleDelayOverride >= 0
        ? Math.floor(cycleDelayOverride)
        : configuredCycleDelay;
    const messageDelay = settings.message_delay_minutes;
    const text = buildPreview(message.content, settings.signature, true);
    /*
    ...
    for (let cycle = 1; cycle <= cycleLimit; cycle++) {
        ...
        for (let index = 0; index < targets.length; index++) {
            ...
            try {
                const inputPeer = await resolveTargetEntity(client, {
                ...
                cycleFailed++;
                totalFailed++;
                console.error("PUBLISH TARGET ERROR:", {
                    runId,
                    cycle,
                    targetId: target.id,
                    title: group.title,
         ...
         */
        console.log(`PUBLISH CYCLE ${cycle}/${cycleLimit}: ` +
 ...
 */
export async function executeSinglePublish(userId, accountId, runId) {
    const result = await executePublishCycles(userId, accountId, runId);
    return {
