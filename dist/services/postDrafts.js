...
export async function createDraft(userId, accountId) {
...
export async function updateDraftContent(userId, draftId, content) {
    const { data, error } = await supabase
        .from("post_drafts")
        .update({
        content,
        ...
        .single();
    if (error || !data) {
        throw new Error(error?.message || "DRAFT_UPDATE_FAILED");
    }
    return data;
}
export async function toggleDraftSignature(userId, draftId) {
    const draft = await getDraft(userId, draftId);
    const { data, error } = await supabase
        .from("post_drafts")
        .update({
        signature_enabled: !draft.signature_enabled,
        updated_at: new Date().toISOString()
    })
        .eq("id", draftId)
        .eq("user_id", userId)
...
export async function toggleDraftGroup(userId, draftId, groupId) {
    const draft = await getDraft(userId, draftId);
...
export async function markDraftUsed(userId, draftId) {
    await supabase
        .from("post_drafts")
        .update({
        status: "used",
