...
export async function setAdminAction(telegramId, action) {
...
export async function getAdminAction(telegramId) {
...
export async function clearAdminAction(telegramId) {
    await supabase
        .from("admin_sessions")
        .delete()
        .eq("telegram_id", telegramId);
...
export async function addMandatoryChannel(input) {
    const clean = input.trim();
    if (!clean) {
        throw new Error("EMPTY_CHANNEL");
    }
    let chatIdOrUsername = clean;
    if (/^@/.test(clean)) {
    ...
    let botMember;
    try {
        botMember = await bot.api.getChatMember(chat.id, me.id);
    }
    catch {
...
export async function deleteMandatoryChannel(id) {
    const { error } = await supabase
        .from("mandatory_channels")
        .delete()
        .eq("id", id);
