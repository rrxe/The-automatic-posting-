...
function parseInput(rawLine) {
    ...
    if (publicLink) {
        return { type: "username", value: `@${publicLink[1]}` };
    }
...
async function resolveOneLine(client, rawLine) {
    const parsed = parseInput(rawLine);
    if (parsed.type === "invite") {
        const preview = await client.invoke(new Api.messages.CheckChatInvite({ hash: parsed.value }));
        if ("chat" in preview && preview.chat) {
            const entity = preview.chat;
            return {
                input: rawLine.trim(),
                entity,
 ...
 */
export async function prepareChatAdd(telegramId, userId, accountId, rawInput) {
    ...
    for (let i = 0; i < batch.length; i++) {
        try {
            ...
            failed.push({
                input: batch[i],
                reason: failureLabel(error instanceof Error ? error.message : "UNKNOWN")
            });
        }
        if (i < batch.length - 1) {
...
async function saveGroup(userId, accountId, entity, activate) {
    ...
    const { data: group, error } = await supabase
        .from("groups")
        .upsert({ telegram_chat_id: telegramChatId, title, username, is_active: true }, { onConflict: "telegram_chat_id" })
        .select("id")
        .single();
    if (error || !group) {
...
export async function confirmChatAdd(telegramId) {
    ...
    for (const entry of item.items) {
        ...
        const saved = await saveGroup(item.userId, item.accountId, entry.entity, activate);
        if (activate) {
            remaining--;
