import { beginLogin, submitLoginCode, submitLoginPassword, cancelLogin } from "../telegram/auth.js";
import { getWebTelegramLoginSession, deleteWebTelegramLoginToken } from "../services/webTelegramLogin.js";
...
function safeError(error) {
    const message = error instanceof Error
        ? error.message
        : String(error);
    if (message.includes("FloodWait") ||
        message.includes("PHONE_NUMBER_FLOOD") ||
    ...
    if (message.includes("PASSWORD_NOT_REQUESTED")) {
        return "كلمة المرور ليست مطلوبة حاليًا.";
    }
    return "تعذر إكمال تسجيل الدخول. حاول مرة أخرى.";
}
export function registerTelegramLoginRoutes(app) {
    app.get("/telegram-login", (_req, res) => {
        res.setHeader("Cache-Control", "no-store");
        res.type("html").send(html());
    });
    app.post("/api/telegram-login/start", async (req, res) => {
        const token = getToken(req);
        const session = getWebTelegramLoginSession(token);
        if (!session) {
            res
                .status(401)
                .json({
                ok: false,
                error: "رابط التسجيل غير صالح أو انتهت صلاحيته."
            });
            return;
        }
        ...
        try {
            const result = await beginLogin(session.userId, session.telegramId, phone);
            res.json({
                ok: true,
                deliveredToApp: result.deliveredToApp
            });
        }
        catch (error) {
            res
    ...
    app.post("/api/telegram-login/code", async (req, res) => {
        ...
        if (!session) {
            res
                ...
                .json({
                ok: false,
                error: "جلسة التسجيل غير صالحة."
            });
            return;
        }
        try {
            const result = await submitLoginCode(session.telegramId, String(req.body?.code ??
                ""));
            if (result.status ===
                "password") {
                res.json({
            ...
            if (result.status ===
                "completed") {
                deleteWebTelegramLoginToken(token);
                res.json({
                    ok: true,
                    status: "completed"
                });
    ...
    app.post("/api/telegram-login/password", async (req, res) => {
        ...
        try {
            const result = await submitLoginPassword(session.telegramId, String(req.body?.password ??
                ""));
            if (result.status ===
                "completed") {
                deleteWebTelegramLoginToken(token);
    ...
    app.post("/api/telegram-login/cancel", (req, res) => {
        const token = getToken(req);
        const session = getWebTelegramLoginSession(token);
        if (session) {
            cancelLogin(session.telegramId);
        }
        deleteWebTelegramLoginToken(token);
        res.json({
            ok: true
