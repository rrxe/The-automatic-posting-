...
const rich = __importStar(require("../../richMessage"));
...
class CustomMessage extends senderGetter_1.SenderGetter {
    ...
    get richText() {
        return this.richMessage && this.richMessage instanceof api_1.Api.RichMessage
     ...
     */
    async fetchRichMessage() {
        const current = this.richMessage;
        if (!this._client ||
            !current ||
            !(current instanceof api_1.Api.RichMessage) ||
            !current.part) {
            return this;
        }
        const full = await this._client.getRichMessage((await this.getInputChat()), this.id);
    ...
    async forwardTo(entity, params) {
        if (this._client) {
            entity = await this._client.getInputEntity(entity);
            return this._client.forwardMessages(entity, Object.assign(Object.assign({}, params), { messages: [this.id], fromPeer: (await this.getInputChat()) }));
        }
    }
    async edit(params) {
        const param = params;
    ...
    async react(reaction, big, addToRecent) {
        if (!this._client)
            return;
        ...
        return this._client.sendReaction((await this.getInputChat()), this.id, reactionList, big, addToRecent);
    }
    async getReactions(limit, reaction, offset) {
        if (!this._client)
            return;
        return this._client.getReactionUsers((await this.getInputChat()), this.id, { limit, reaction, offset });
    }
    async copy(entity, params) {
        if (!this._client)
            return;
        entity = await this._client.getInputEntity(entity);
        const sent = await this._client.forwardMessages(entity, Object.assign(Object.assign({}, params), { messages: [this.id], fromPeer: (await this.getInputChat()), dropAuthor: true }));
        return sent[0];
    }
